...
int main(int argc, char *argv[]) {
    ...
    CudnnPoolDriver driver(CudnnPoolForm::BACKWARD_AVGNOPAD, problems, options.getNumRuns(),
                            options.getGpus());

    printf("Starting Backward Pool Average without Pad through %s set with %d runs\n", options.getProblemSetFile().c_str(), 
            options.getNumRuns());
