...
int main(int argc, char *argv[]) {
    ...
    CudnnSoftmaxDriver driver(CudnnSoftmaxForm::BACKWARD_ACCURATE, problems, options.getNumRuns(),
                            options.getGpus());

    printf("Starting Backward Softmax Accurate through %s set with %d runs\n", options.getProblemSetFile().c_str(), 
            options.getNumRuns());
