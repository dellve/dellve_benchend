...
int main(int argc, char *argv[]) {
    ...
    CudnnPoolDriver driver(CudnnPoolForm::FORWARD_AVGNOPAD, problems, options.getNumRuns(),
                            options.getGpus());

    printf("Starting Forward Pool Average without Padding through %s set with %d runs\n", options.getProblemSetFile().c_str(), 
            options.getNumRuns());
