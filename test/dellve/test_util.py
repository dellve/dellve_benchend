...
def test_api_url(host, port):
    url = dellve.util.api_url('%s/%d/url', 'test', 0,
                              host=host, port=port)
...
def test_api_get_benchmark(host, port):
    ...
    responses.add(responses.GET, 'http://%s:%d/benchmark' % (host, port),
                  body=json.dumps(data), content_type='application/json', status=200)

    assert dellve.util.api_get('benchmark') == data
...
def test_api_get_benchmark_error(host, port):
    ...
    responses.add(responses.GET, 'http://%s:%d/benchmark' % (host, port),
                  body=json.dumps(data), content_type='application/json', status=500)
...
def test_api_get_benchmark_progress(host, port):
    ...
    assert dellve.util.api_get('benchmark/progress') == data
...
def test_api_get_benchmark_start(host, port):
    ...
    assert dellve.util.api_post('benchmark/%d/start', 0) == data
...
def test_api_get_benchmark_stop(host, port):
    ...
    assert dellve.util.api_post('benchmark/%d/stop', 0) == data
